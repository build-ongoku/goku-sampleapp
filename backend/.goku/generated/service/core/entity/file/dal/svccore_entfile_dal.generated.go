...
func (c Client) Add(ctx context.Context, req svccore_entfile_typ.AddRequest) (svccore_entfile_typ.File, error) {
	llog.Info(ctx, "Add entity", "entity", "File", "request", req)
...
func (c Client) AddBatch(ctx context.Context, reqs ...svccore_entfile_typ.AddRequest) ([]svccore_entfile_typ.File, error) {
	llog.Info(ctx, "AddFiles: Adding entities...", "entity", "File", "request", reqs)
...
func (c Client) Get(ctx context.Context, req svccore_entfile_typ.GetRequest) (svccore_entfile_typ.File, error) {
	llog.Info(ctx, "Get entity", "entity", "File", "request", req)
	id := req.ID
...
func (c Client) List(ctx context.Context, req svccore_entfile_typ.ListRequest) (svccore_entfile_typ.ListResponse, error) {
	llog.Info(ctx, "List entities", "entity", "File", "request", req)
	var resp svccore_entfile_typ.ListResponse
...
func (c Client) Update(ctx context.Context, req svccore_entfile_typ.UpdateRequest) (dalutil.UpdateEntityResponse[svccore_entfile_typ.File], error) {
	llog.Info(ctx, "Update entity", "entity", "File", "request", req)
...
func (d DAL) ConstructListQueryBuilderForTypeFile(ctx context.Context, conn db.Connection, req dalutil.ListTypeRequest[svccore_entfile_typ.FileFilter], params db.UniqueIDsQueryBuilderParams) (db.SelectQueryBuilder, error) {
	...
	if req.Filter.FileName != nil { // inject a Where filter for FileName
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.FileName, ds, "file_name", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.StorageClient != nil { // inject a Where filter for StorageClient
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.StorageClient, ds, "storage_client", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.StorageClientIdentifier != nil { // inject a Where filter for StorageClientIdentifier
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.StorageClientIdentifier, ds, "storage_client_identifier", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.SizeBytes != nil { // inject a Where filter for SizeBytes
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.SizeBytes, ds, "size_bytes", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.MimeType != nil { // inject a Where filter for MimeType
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.MimeType, ds, "mime_type", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.FileHash != nil { // inject a Where filter for FileHash
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.FileHash, ds, "file_hash", false)
		if err != nil {
			return resp, err
		}
	}
