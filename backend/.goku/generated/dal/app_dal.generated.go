...
func (d DAL) ConstructListQueryBuilderForTypeAddress(ctx context.Context, conn db.Connection, req dalutil.ListTypeRequest[app_typ.AddressFilter], params db.UniqueIDsQueryBuilderParams) (db.SelectQueryBuilder, error) {
	...
	if req.Filter.Line1 != nil { // inject a Where filter for Line1
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.Line1, ds, "line_1", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.Line2 != nil { // inject a Where filter for Line2
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.Line2, ds, "line_2", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.City != nil { // inject a Where filter for City
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.City, ds, "city", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.State != nil { // inject a Where filter for State
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.State, ds, "state", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.PostalCode != nil { // inject a Where filter for PostalCode
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.PostalCode, ds, "postal_code", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.Country != nil { // inject a Where filter for Country
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.Country, ds, "country", false)
		if err != nil {
			return resp, err
		}
	}
...
func (d DAL) ConstructListQueryBuilderForTypePersonName(ctx context.Context, conn db.Connection, req dalutil.ListTypeRequest[app_typ.PersonNameFilter], params db.UniqueIDsQueryBuilderParams) (db.SelectQueryBuilder, error) {
	...
	if req.Filter.FirstName != nil { // inject a Where filter for FirstName
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.FirstName, ds, "first_name", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.MiddleInitial != nil { // inject a Where filter for MiddleInitial
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.MiddleInitial, ds, "middle_initial", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.LastName != nil { // inject a Where filter for LastName
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.LastName, ds, "last_name", false)
		if err != nil {
			return resp, err
		}
	}
...
func (d DAL) ConstructListQueryBuilderForTypePhoneNumber(ctx context.Context, conn db.Connection, req dalutil.ListTypeRequest[app_typ.PhoneNumberFilter], params db.UniqueIDsQueryBuilderParams) (db.SelectQueryBuilder, error) {
	...
	if req.Filter.CountryCode != nil { // inject a Where filter for CountryCode
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.CountryCode, ds, "country_code", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.Number != nil { // inject a Where filter for Number
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.Number, ds, "number", false)
		if err != nil {
			return resp, err
		}
	}

	if req.Filter.Extension != nil { // inject a Where filter for Extension
		ds, err = filterlib.InjectConditionIntoSqlBuilder(req.Filter.Extension, ds, "extension", false)
		if err != nil {
			return resp, err
		}
	}
