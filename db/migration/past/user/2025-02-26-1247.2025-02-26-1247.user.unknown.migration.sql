...
CREATE TABLE public.tb_role_permissions (
    parent_id uuid,
    id uuid NOT NULL,
    entity text,
    type text,
...
ALTER TABLE public.tb_role ADD CONSTRAINT chk_role__name__empty_str CHECK (name <> ''::TEXT);

ALTER TABLE public.tb_role ADD CONSTRAINT pk_role_id PRIMARY KEY (id);

ALTER TABLE public.tb_role_permissions ADD CONSTRAINT chk_permissions_entity_enum CHECK (entity = ANY (ARRAY['Auth_Secret'::text, 'Core_File'::text, 'Core_SecretDecryptable'::text, 'Core_SecretKey'::text, 'User_Device'::text, 'User_Group'::text, 'User_Role'::text, 'User_User'::text]));

ALTER TABLE public.tb_role_permissions ADD CONSTRAINT chk_permissions_type_enum CHECK (type = ANY (ARRAY['Read'::text, 'Write'::text]));

ALTER TABLE public.tb_role_permissions ADD CONSTRAINT pk_role_permissions_id PRIMARY KEY (id);
...
ALTER TABLE public.tb_role_permissions ADD CONSTRAINT fk_role_permissions__parent_id__role FOREIGN KEY (parent_id) REFERENCES public.tb_role (id) ON DELETE CASCADE;
